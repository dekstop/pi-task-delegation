...
**Next:** (1) done — devDeps installed pinned to runtime versions, `typecheck` script added. (2) done — `tsc --noEmit` clean (added 2× `details: undefined` in index.ts). (3) done — `vitest run` 37/37 green. (4) **Commit T-004** (`tool.ts` + `index.ts` + tests + type-check setup + task/T-007). Then T-005/T-006.
