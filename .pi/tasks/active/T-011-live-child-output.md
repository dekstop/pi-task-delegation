...
**Next:** (1) `executor.ts`: add `live` + `scheduleFlush` (100 ms), `session.subscribe` listener for `tool_execution_start` + `text_delta`, final flush after `prompt`, unsubscribe + timer clear in `finally`; `emit("starting…")` before `prompt`. (2) `index.ts`: `renderCall` + `renderResult` (T-010). (3) `npx vitest run` (baseline 39). (4) Manual TUI check: status line + streaming transcript while a delegate runs, final result on completion.
